...
#       endif
    ...
    void Awake()
    {
        ...
        if (!gua.analyticsDisabled)
        {
            ...
            {
                sendSystemInfoEvent(category, "ScreenResolution", "" + Screen.currentResolution.width + "x" + Screen.currentResolution.height,-1);
                sendSystemInfoEvent(category, "ViewportSize", "" + Screen.width + "x" + Screen.height, -1);
                sendSystemInfoEvent(category, "ScreenDPI", ((int)Screen.dpi).ToString(), (int)Screen.dpi);

                sendSystemInfoEvent(category, "operatingSystem", SystemInfo.operatingSystem, -1);
                sendSystemInfoEvent(category, "processorType", SystemInfo.processorType, -1);
                ...
                sendSystemInfoEvent(category, "graphicsDeviceName", SystemInfo.graphicsDeviceName, -1);
                sendSystemInfoEvent(category, "graphicsDeviceVendor", SystemInfo.graphicsDeviceVendor, -1);
                ...
                sendSystemInfoEvent(category, "graphicsDeviceVersion", SystemInfo.graphicsDeviceVersion,-1);
...
#               endif // Unity 5.0+
                sendSystemInfoEvent(category, "deviceType", SystemInfo.deviceType.ToString(), -1);
...
#               if !UNITY_3_5
                ...
                sendSystemInfoEvent(category, "npotSupport", SystemInfo.npotSupport.ToString(), -1);
#               endif
...
#               endif
                ...
                sendSystemInfoEvent(category, "deviceModel", SystemInfo.deviceModel, -1);
...
#           endif
    ...
    public void sendSystemInfoEvent(string eventCategory, string eventAction, string eventLabel , int eventValue )
    {
        if (gua.analyticsDisabled)
            return;
