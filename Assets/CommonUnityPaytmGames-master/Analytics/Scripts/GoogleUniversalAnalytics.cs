...
#       endif // unity standalone win
        ...
        if (!String.IsNullOrEmpty(forceUserAgent))
        {
            if (unityMajorVersion == 2017 && unityMinorVersion < 3)
            {
                ...
                setUserAgentOverride(forceUserAgent,false);
#               if GUA_DEBUG_LOGS
...
#       endif // debug logs
    ...
    public void setAnonymizeIP(bool enabled)
    {
        anonymizeIP = enabled;
    }
...
#       endif // debug warnings
     ...
     */
    public void setUserAgentOverride(string userAgent, bool allowNonEscaped )
    {
     ...
     */
    public void addDocumentReferrer(string url, bool allowNonEscaped)
    {
#       if UNITY_WEBPLAYER || UNITY_WEBGL
        if (url == null)
        {
...
#       endif // debug warnings
     ...
     */
    public void addCampaignKeyword(string text, bool allowNonEscaped)
    {
...
#       endif // debug warnings
     ...
     * \param allowNonEscaped text is always escaped, unless this is set to true.
     */
    public void addCampaignContent(string text, bool allowNonEscaped)
    {
        string data = allowNonEscaped ? escapeString(text) : GUAWebRequest.EscapeURL(text);
        sb.Append("&cc=");
        sb.Append(data);
...
#           endif // debug warnings
     ...
     * \param allowNonEscaped url is always escaped, unless this is set to true.
     */
    public void addDocumentLocationURL(string url, bool allowNonEscaped )
    {
        string data = allowNonEscaped ? escapeString(url) : GUAWebRequest.EscapeURL(url);
        sb.Append("&dl=");
        sb.Append(data);
...
#       endif // debug warnings
     ...
     */
    public void addDocumentPath(string text, bool allowNonEscaped)
    {
        string data = allowNonEscaped ? escapeString(text) : GUAWebRequest.EscapeURL(text);
        sb.Append("&dp=");
        sb.Append(data);
...
#       endif // debug warnings
     ...
     * \param allowNonEscaped text is always escaped unless this is set to true.
     */
    public void addSocialActionTarget(string text, bool allowNonEscaped)
    {
#       if GUA_DEBUG_WARNINGS
        if (hitType != HitType.Social)
...
#       endif
        ...
        addDocumentPath(documentPath,false);
     ...
     * \sa addEventValue
     */
    public void sendEventHit(string eventCategory, string eventAction, string eventLabel, int eventValue)
    {
        if (analyticsDisabled)
            return;
#       if GUA_DEBUG_WARNINGS
...
#       endif
        ...
        addSocialActionTarget(target,false);
        sendHit();
    }
...
#               endif
    ...
    private GUAWebRequest finalizeAndSendHit(bool needReturnGWR)
    {
